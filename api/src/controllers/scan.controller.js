    ...
    try {
        ...
        if (address) {
            ...
            if (response.data.status !== '1' || !response.data.result[0].SourceCode) {
                return res.status(404).json({ error: 'Failed to fetch from Etherscan' });
            }
        ...
        return res.status(400).json({ error: 'Please provide either a contract address or upload a .sol file' });
